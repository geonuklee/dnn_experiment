...
class Sub:
    ...
    def callback(self, cloud):
        ...
        rgb_field = ros_numpy.point_cloud2.split_rgb_field(array)
        rgb = np.vstack((rgb_field['r'], rgb_field['g'], rgb_field['b'])).transpose()
        xyz = np.vstack((array['x'],array['y'],array['z'])).transpose()
        self.xyz_rgb = np.hstack( (xyz, rgb) ).astype(np.float)
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        rate.sleep()
        if sub.xyz_rgb is None:
            continue
        xyz_rgb = np.array(sub.xyz_rgb, order='C')
        bat_pc = cpp_ext.normalize_pointcloud(xyz_rgb)
