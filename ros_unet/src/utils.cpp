...
cv::Mat Overlap(cv::Mat bg, cv::Mat mask, float alpha, bool put_text) {
  cv::Mat colored_bg;
  if(bg.channels() > 1)
    colored_bg = bg;
  else
...
void ColorizeSegmentation(const std::map<int, boost::shared_ptr<pcl::PointCloud<pcl::PointXYZL> > >& clouds,
                          sensor_msgs::PointCloud2& msg){
  ColorizeSegmentation<pcl::PointXYZL>(clouds, msg);
  return;
}

void ColorizeSegmentation(const std::map<u_int32_t, boost::shared_ptr<pcl::PointCloud<pcl::PointXYZL> > >& _clouds,
                          sensor_msgs::PointCloud2& msg){
  std::map<int, boost::shared_ptr<pcl::PointCloud<pcl::PointXYZL> > > clouds;
  for(auto it:_clouds)
    clouds[it.first] = it.second;
  ColorizeSegmentation<pcl::PointXYZL>(clouds, msg);
  return;
}
