...
if __name__=="__main__":
    ...
    # No rosparam for each rosbag,..

    for gt_fn in gt_files:
        pick = get_pick(gt_fn)
        bag = rosbag.Bag(pick['fullfn'])
        #cam_id = get_camid(gt_fn) # 'cam0'
        ...
        for topic, msg, t in bag.read_messages(topics=rgb_topics.values()+depth_topics.values()):
            ...
            resp = predict_edge(rect_depth_msg, fx, fy)
            compute_obb(rect_depth_msg, rect_rgb_msg, resp.mask, Twc, std_msgs.msg.String(cam_id))
            # t1
            ...
            cvedge = np.frombuffer(resp.mask.data, dtype=np.uint8).reshape(resp.mask.height, resp.mask.width, 2)
